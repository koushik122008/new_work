...
// Reusable Geometries and Materials for optimization
let sharedProjectileGeometry;
let sharedProjectileMaterial;
let sharedEnemyCoreGeometry;
let sharedEnemyCoreMaterial;
let sharedEnemySpikeGeometry;
let sharedEnemySpikeMaterial;
...
function init() {
    ...
    // Pre-allocate shared geometries and materials
    sharedProjectileGeometry = new THREE.CylinderGeometry(0.1, 0.1, 6, 8);
    sharedProjectileGeometry.rotateX(Math.PI / 2);
    sharedProjectileMaterial = new THREE.MeshBasicMaterial({ color: 0xffaa00 });

    sharedEnemyCoreGeometry = new THREE.DodecahedronGeometry(2);
    sharedEnemyCoreMaterial = new THREE.MeshStandardMaterial({
        map: enemyTexture,
        color: 0xaa0000,
        metalness: 0.8,
        roughness: 0.2
    ...
    sharedEnemySpikeGeometry = new THREE.ConeGeometry(0.5, 3, 4);
    sharedEnemySpikeMaterial = new THREE.MeshStandardMaterial({
        map: metalTexture,
        color: 0x333333,
        metalness: 0.9,
        roughness: 0.5
...
function fireProjectile() {
    // Projectile visual - glowing laser tracer (using shared geometry and material)
    const projectile = new THREE.Mesh(sharedProjectileGeometry, sharedProjectileMaterial);
...
function spawnEnemy() {
    ...
    // Core body
    const core = new THREE.Mesh(sharedEnemyCoreGeometry, sharedEnemyCoreMaterial);
    core.castShadow = true;
    enemy.add(core);

    // Armor plates/spikes
    for (let i = 0; i < 4; i++) {
        const spike = new THREE.Mesh(sharedEnemySpikeGeometry, sharedEnemySpikeMaterial);
