...
// Shared Pre-allocated Geometries & Materials (Performance Optimization)
...
function fireProjectile() {
    // Projectile visual - glowing laser tracer (using pre-allocated shared objects)
...
function spawnEnemy() {
    ...
    // Core body (using pre-allocated shared objects)
    const core = new THREE.Mesh(sharedEnemyCoreGeo, sharedEnemyCoreMat);
    core.castShadow = true;
    enemy.add(core);

    // Armor plates/spikes (using pre-allocated shared objects)
    for (let i = 0; i < 4; i++) {
