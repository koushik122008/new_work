...
// Shared Assets to prevent GC overhead
...
function fireProjectile() {
    // Projectile visual - glowing laser tracer (using shared assets)
...
function spawnEnemy() {
    ...
    // Core body (using shared assets)
    const core = new THREE.Mesh(sharedEnemyCoreGeo, sharedEnemyCoreMat);
    core.castShadow = true;
    enemy.add(core);

    // Armor plates/spikes (using shared assets)
    for (let i = 0; i < 4; i++) {
