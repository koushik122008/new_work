...
function init() {
    ...
    // Pre-allocate shared geometries and materials for performance (Bolt)
...
function fireProjectile() {
    // Projectile visual - glowing laser tracer using shared assets (Bolt)
...
function spawnEnemy() {
    ...
    // Core body using shared assets (Bolt)
    const core = new THREE.Mesh(sharedEnemyCoreGeo, sharedEnemyCoreMat);
    core.castShadow = true;
    enemy.add(core);

    // Armor plates/spikes using shared assets (Bolt)
    for (let i = 0; i < 4; i++) {
